...
hero:
  ...
  actions:
    ...
    - text: Stars on GitHub
      link: https://github.com/cloudticon/ct
      icon: github
      variant: secondary
---
...
Port forwarding, file sync, log streaming — all start in parallel. Edit a file locally and it lands in the container instantly. No Docker rebuilds, no separate YAML config. [Full CT Dev docs →](/ct-dev/overview/)
...
Want less boilerplate for common single-container workloads? Use [k8s-factories](https://github.com/cloudticon/k8s-factories) and start from `webApp()` plus optional `expose()` routing.
...
This single call creates `Deployment` + `Service`, and can also add HPA, PVC-backed volumes, and inline exposure. For more patterns, see [Examples](/patterns/examples/).
...
- **[K8s Resource Factories](/patterns/base-primitive/)** — reusable `resource()` based APIs.
- **[K8s High-level Factories](/patterns/examples/)** — `webApp()` and `expose()` patterns from `k8s-factories`.
