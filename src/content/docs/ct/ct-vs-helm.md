...
### Helm — conditional with nested helpers
...
Typo in `replicaCount`? Helm won't tell you — it silently uses the default.
...
Typo? Immediate red squiggly in VS Code. No deploy needed.
...
Updating a subchart means `helm dependency update`, version pinning, repository registration. Breaking changes cascade through `values.yaml` overrides.

### CT — Git imports

```ts
import { createPostgres } from "github.com/my-org/k8s-db@v2.0.0/postgres";
import { createRedis } from "github.com/my-org/k8s-cache@v1.3.0/redis";

createPostgres({ name: "db", instances: 2 });
createRedis({ name: "cache", size: "1Gi" });
```

Standard ES imports. Pin to a tag. Update with a one-line version bump. No registry, no Chart.yaml, no dependency update dance.
